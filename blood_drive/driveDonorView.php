...
$sql = "SELECT drive_booking.blood_drive_id,drive_booking_id,blood_drive_name,blood_drive_location,date_from,date_to,hospital_name FROM 
blood_drive,hospital,drive_booking WHERE hospital.hospital_id = blood_drive.hospital_id AND blood_drive.blood_drive_id = drive_booking.blood_drive_id 
AND  drive_booking.donor_id = '$id'AND date_to > '$current_date' ORDER BY date_from";
...
<html>
 ...
 <main class="donor_land">
    ...
    <div class="container">
            ...
            <h1 style = 'text-align:center; margin-top:15px;'>My Registered Blood Drives</h1>
                ...
                <?php
                    if (mysqli_num_rows($result) > 0)
                    {
                        while($row = mysqli_fetch_assoc($result))
                        {
                            ...
                            ?><form method = 'post' action = "../connection/cancelDriveBooking.php?id=<?php echo $drive_booking_id;?>"><div id = 'driveview'class="col-md-auto box-shadow" ><?php
                            echo "<h3 style = 'text-align:center'>".$row['blood_drive_name']."</h3>
                            <b><p>Booking ID:</b> ".$row['drive_booking_id']."</p>
                            ...
                            <div style = 'text-align:center'>
                                <a href = ""><input style = 'color:white;background-color:black;width:auto' type = 'submit' value = 'Cancel' name = 'cancel'></a>
                            </div>
                            </div></form><?php
                        }
                    }else{
                        echo "<div class = 'col'></div><div style = 'text-align:center; margin-top: 20px;margin-bottom: 371px'class = 'col-md-auto'><h5>You have not registered for any upcoming blood drive. <br/></h5><a href = 'viewDrives.php'>View available blood drives?</a></div><div class = 'col'></div>";
                    }
                ?>
            </div>
        </div>
